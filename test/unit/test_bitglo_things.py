...
@pytest.fixture
def valid_bitglo_address(network='mainnet'):
    return 'n7mzEvBy6JdP2QQUqJrLjitucQKgTBZAR2' if (network == 'testnet') else 'BJjGZTPnsbgoU5uLVhPnR3HCt5v4qrf4Zo'


@pytest.fixture
def invalid_bitglo_address(network='mainnet'):
    return 'n7mzEvBy6JdP2QQUqJrLjitu1QKgTBZFZ3' if (network == 'testnet') else 'BJjGZTPnsbgoU5uLVhPnR3HCt5v4qrf4Zo'
...
def mn_status_good():
    ...
    status = {
        ...
        "pubkey": "BJjGZTPnsbgoU5uLVhPnR3HCt5v4qrf4Zo",
...
def test_valid_bitglo_address():
    from bitglolib import is_valid_bitglo_address

    main = valid_bitglo_address()
    test = valid_bitglo_address('testnet')

    assert is_valid_bitglo_address(main) is True
    assert is_valid_bitglo_address(main, 'mainnet') is True
    assert is_valid_bitglo_address(main, 'testnet') is False

    assert is_valid_bitglo_address(test) is False
    assert is_valid_bitglo_address(test, 'mainnet') is False
    assert is_valid_bitglo_address(test, 'testnet') is True


def test_invalid_bitglo_address():
    from bitglolib import is_valid_bitglo_address

    main = invalid_bitglo_address()
    test = invalid_bitglo_address('testnet')

    assert is_valid_bitglo_address(main) is False
    assert is_valid_bitglo_address(main, 'mainnet') is False
    assert is_valid_bitglo_address(main, 'testnet') is False

    assert is_valid_bitglo_address(test) is False
    assert is_valid_bitglo_address(test, 'mainnet') is False
    assert is_valid_bitglo_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from bitglolib import elect_mn
...
def test_parse_masternode_status_vin():
    from bitglolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % bitglolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(bitglolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(bitglolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(bitglolib.blocks_to_seconds(16616)) == 2612035
