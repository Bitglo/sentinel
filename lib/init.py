...
def has_bitglo_conf():
    ...
    valid_bitglo_conf = False

    # ensure bitglo_conf exists & readable
    #
    # if not, print a message stating that Bitglo Core must be installed and
    # configured, including JSONRPC access in bitglo.conf
    try:
        f = io.open(config.bitglo_conf)
        valid_bitglo_conf = True
    except IOError as e:
        print(e)

    return valid_bitglo_conf
...
def main():
    ...
    if not has_bitglo_conf():
        print("BitgloCore must be installed and configured, including JSONRPC access in bitglo.conf")
        sys.exit(1)
