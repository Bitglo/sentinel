...
def is_valid_bitglo_address(address, network='mainnet'):
    ...
    bitglo_version = 112 if network == 'testnet' else 25
    ...
    try:
        ...
        # rescue from exception, not a valid Bitglo address
        return False

    if (address_version != bitglo_version):
        return False
...
# shims 'til we can fix the bitglod side
def SHIM_serialise_for_bitglod(sentinel_hex):
    ...
    # superblock => "trigger" in bitglod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # bitglod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    bitglod_hex = serialise(obj)
    return bitglod_hex


# shims 'til we can fix the bitglod side
def SHIM_deserialise_from_bitglod(bitglod_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(bitglod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('bitglo.conf')
