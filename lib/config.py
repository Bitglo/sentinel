...
sentinel_cfg = BitgloConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_bitglod_proto_version_with_sentinel_ping = 70207


def get_bitglo_conf():
    home = os.environ.get('HOME')

    bitglo_conf = os.path.join(home, ".bitglocore/bitglo.conf")
    if sys.platform == 'darwin':
        bitglo_conf = os.path.join(home, "Library/Application Support/BitgloCore/bitglo.conf")

    bitglo_conf = sentinel_cfg.get('bitglo_conf', bitglo_conf)
