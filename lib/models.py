...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bitglod):
        golist = bitglod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_bitglod(bitglod, item)
    ...
    @classmethod
    def import_gobject_from_bitglod(self, bitglod, rec):
        import decimal
        import bitglolib
        import inflection
        ...
        # shim/bitglod conversion
        object_hex = bitglolib.SHIM_deserialise_from_bitglod(object_hex)
        objects = bitglolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from bitglod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bitglod, with every run
        ...
        # get/create, then sync payment amounts, etc. from bitglod - Bitglod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bitglod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bitglod! %s" % e)
            govobj.vote_delete(bitglod)
            return (govobj, None)
    ...
    def vote_delete(self, bitglod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bitglod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bitglod, signal, outcome):
        import bitglolib
        ...
        output = bitglod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bitglolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bitglod, signal)

    def sync_network_vote(self, bitglod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bitglod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bitglo addr, non-multisig
            if not bitglolib.is_valid_bitglo_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Bitglo address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bitglolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BitgloDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import bitglolib
        obj_data = bitglolib.SHIM_serialise_for_bitglod(self.serialise())
    ...
    def prepare(self, bitglod):
        try:
            object_hash = bitglod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bitglolib.is_valid_bitglo_address(addr, config.network):
    ...
    def hash(self):
        import bitglolib
        return bitglolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, bitglod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - bitglod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, bitglod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - bitglod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, bitglod):
        now = int(time.time())
        return (self.created_at < (now - bitglod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, bitglod):
        if self.is_expired(bitglod):
            return False

        return True

    def is_deletable(self, bitglod):
        if self.is_expired(bitglod):
            return True
