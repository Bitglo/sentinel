...
class BitgloDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some bitglod methods
    ...
    @classmethod
    def from_bitglo_conf(self, bitglo_dot_conf):
        from bitglo_config import BitgloConfig
        config_text = BitgloConfig.slurp_config_file(bitglo_dot_conf)
        creds = BitgloConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = bitglolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = bitglolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = bitglolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_bitglod_proto_version_with_sentinel_ping)
